...
```
그러나, 클러스터에서 실행하는 경우 local 패스로 잡혀있는 spark-examples_2.12-3.3.1.jar 를 찾지 못해서 오류가 발생한다. 
...
#### 1. spark 컨테이너 이미지에 jar 파일 넣기 ####
이 경우 스파크 컨테이너를 다시 만들어야 한다. 이때 cp 명령어를 이용하여 spark-examples_2.12-3.3.1.jar 파일을 컨테이너 이미지에 포함시켜야 한다. 
Docker file 을 새로 만들어야 한다. ***
