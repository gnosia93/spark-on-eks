...
local:// 을 이용하여 절대 경로를 주어도 실행가능 하다. local:// 의 경우 컨테이너 이미지 내의 경로에서 jar 파일을 불러온다 (절대경로만 지원됨)
```
$ ./bin/spark-submit \
    --master local \
    --name spark-pi \
    ...
    --conf spark.executor.instances=5 \
    --conf spark.kubernetes.container.image=public.ecr.aws/o5l1c9o9/spark-scala-container:latest \
    local:///Users/soonbeom/spark-3.3.1-bin-hadoop3/examples/jars/spark-examples_2.12-3.3.1.jar
```
